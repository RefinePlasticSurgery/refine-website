...
export const getPricingByCategory = (category: string): PricingItem[] =>
  pricingData.filter(item => item.category === category);

/**
 * Single source of truth for procedure names, grouped by category.
 * Used by Contact form dropdown — do NOT maintain a separate list elsewhere.
 */
export const proceduresByCategory: Record<string, string[]> = Object.fromEntries(
  pricingCategories.map(cat => [
    cat,
    getPricingByCategory(cat).map(item => item.name),
  ])
);

/**
 * Format a TZS price amount for display.
 * If your pricing policy is "contact for price" just don't call this —
 * display your own label instead of passing a number and ignoring it.
 */
export const formatPrice = (amount: number): string =>
  `TZS ${amount.toLocaleString('en-TZ')}`;

/**
 * Contact-page label — clinics often display this instead of exact prices.
 */
export const PRICE_LABEL_CONTACT = 'Contact for Price' as const;
