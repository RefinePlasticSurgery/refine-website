...
import { proceduresByCategory } from "@/lib/pricing-data";
...
export const Contact = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ name: "", email: "", phone: "", procedure: "", date: "", message: "" });
    } catch (error: unknown) {
      // ✅ FIX: single error-processing pass — no pipe-string encoding hack
      ...
      const title = titleMap[errorDetails.type] ?? "Request Failed";

      captureException(err, { source: "Contact.handleSubmit" });
      console.error("Error sending appointment request:", err);

      toast({ title, description: errorDetails.message, variant: "destructive" });
  ...
  // ✅ Single source of truth — derived from pricing-data.ts, not duplicated here.
  // To add / rename a procedure, edit src/lib/pricing-data.ts only.
  const dropdownCategories = [
    ...Object.entries(proceduresByCategory).map(([category, procedures]) => ({
      category,
      procedures,
    })),
    { category: "Other Procedures", procedures: ["Other/Consultation Only"] },
  ];
